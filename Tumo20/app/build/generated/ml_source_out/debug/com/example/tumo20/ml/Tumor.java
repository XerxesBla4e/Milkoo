...
public final class Tumor {
  ...
  private Tumor(@NonNull Context context, @NonNull Model.Options options) throws IOException {
    model = Model.createModel(context, "tumor.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static Tumor newInstance(@NonNull Context context) throws IOException {
    return new Tumor(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static Tumor newInstance(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    return new Tumor(context, options);
  }
